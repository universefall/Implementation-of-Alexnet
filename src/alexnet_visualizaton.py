...
if __name__ == "__main__":
    ...
    kernel_num = -1
    vis_max = 1 # alexnet 一共包含5个卷积层[0,1,2,3,4]，现在可视化前两个卷积层的核
    for sub_module in alexnet.modules():
        ...
        c_out, c_int, k_h, k_w = tuple(kernels.shape) # 结果为输出通道数，输入通道数，kernel的长宽

        # 拆分channel
        for o_idx in range(c_out):
            kernel_idx = kernels[o_idx, :, :, :].unsqueeze(1)  # 获得(3, h, w), 但是make_grid需要 BCHW，这里拓展C维度变为（3， 1， h, w）
            '''utils.make_grid(tensor,):用于制作网格图片可视化,tensor shape: Batch * C * H * W,
               normalize：像素标准化到[0,1], scale_each:单张像素值标准化'''
