...
import sys
import time
sys.path.append(os.pardir) # 添加父文件夹路径
import numpy as np
...
if __name__ == "__main__":
    ...
    alexnet_model.classifier._modules["6"] = nn.Linear(num_ftrs, num_classes) # 修改网络最后一层输出为2分类
...
# ============================ step 5/5 训练 ============================
        ...
        if (epoch+1) % val_interval == 0:
            ...
            with torch.no_grad():
                for j, data in enumerate(valid_loader):
                    ...
                    outputs = alexnet_model(inputs.view(-1, c, h, w)) # inputs -> [40,3,224,224]
                    outputs_avg = outputs.view(bs, ncrops, -1).mean(1) # outputs:[40,2]->[4,10,2]
    ...
    time_toc = time.time()# 计时结束

    print("training consuming:{:.2f}s".format(time_toc - time_tic)) # 计算训练耗时
